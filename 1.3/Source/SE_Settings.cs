...
namespace SquirtingElephant.ConfigurableRoomStats
{
    public class SE_Settings : Mod
    {
        ...
        public override void DoSettingsWindowContents(Rect settingsWindowSizeRect)
        {
            ...
            Widgets.Label(new Rect(0f, BUTTON_HEIGHT, settingsWindowSizeRect.width, ROW_HEIGHT), "SECRS_AllSettingsAppliedLive".TC());

            Rect scrollViewRect = new Rect(
                settingsWindowSizeRect.x + PADDING_HORIZONTAL,
                settingsWindowSizeRect.y,
                settingsWindowSizeRect.width - 2 * PADDING_HORIZONTAL,
                1800f);
            Widgets.BeginScrollView(new Rect(0, SCROLL_AREA_OFFSET_TOP, settingsWindowSizeRect.width, settingsWindowSizeRect.height - SCROLL_AREA_OFFSET_TOP), ref ScrollPosition, scrollViewRect);
